...
from .misc            import object_to_tuple, query_to_dict, calc_weighted_difference, to_bucket, parse_headers, parse_file, parse_score_file
from .types           import OutputMethod, Params, Policy, XSSConfidence, UrlType, HTTPMethod, FuzzerException, CFGTuple, CFG
from .FileScore       import SCORE
...
VULNERABILITY_SCORE_RWEIGHT = -0.10 ##zczj
...
class Node:
    def __init__(self,
        ...
        self._vul_score: int = 0    ##zczj vulnerability score
    ...
    def vul_score(self):
        # vulnerability score
        len_vul_score = len(self._vul_score)
        imxl_len = len_vul_score * (len_vul_score + 1) / 2
        node_vul_score = 0
        for i in range(len_vul_score):
            node_vul_score += self._vul_score[i] * (i + 1) / imxl_len
        return node_vul_score
    ...
    def parse_instrumentation(self, 
        ...
        if instrument_args.output_method == OutputMethod.HTTP:
            ...
            iterator = parse_file("p." + worker_id)
            vul_file_iterator = parse_score_file("/tmp/instr/scores." + worker_id) ##zczj
        ...
        if instrument_args.policy == Policy.EDGE or \
           instrument_args.policy == Policy.NODE:
            ...
            if instrument_args.policy == Policy.EDGE:
                cfg_xor = cfg
                for s in vul_file_iterator:    ##zczj
                    file = os.path.relpath(s, '/www/admin/localhost_80/wwwroot/wordpress_instr1')
                    file = file.replace('/', '\\')
                    if file in SCORE:
    ...
    ##zc## 可以修改这里的规则，以改进种子调度的顺序；
    ##zc## 我的想法是把漏洞分加进来，即在return中再加一个calc_weighted_difference()，计算漏洞分差值
    ##zc## 需要在该类中添加漏洞分对应的变量，并根据种子经过的基本块更新漏洞分
    def __cmp__(self, node2: Node) -> float:
        ...
        return calc_weighted_difference(node2.cover_score_raw,      self.cover_score_raw,      COVER_SCORE_RWEIGHT)    + \
               ...
               calc_weighted_difference(node2.vul_score,            self.vul_score,            VULNERABILITY_SCORE_RWEIGHT)     ##zczj
